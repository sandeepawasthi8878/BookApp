...
const Cart = () => {
  const [Cart, setCart] = useState([]); // Set initial state as an empty array
  ...
  useEffect(() => {
    if (Cart && Cart.length > 0) {
      ...
      Cart.forEach((item) => {
        let itemPrice = parseFloat(item.price.replace(/[^\d.-]/g, '')); // Remove any non-numeric characters like ₹
        if (!isNaN(itemPrice)) {
          total += itemPrice;
        }
      });
  
      console.log("Total before setting state:", total);
      setTotal(total);
    } else {
...
try{
    const response = await axios.post(
      "http://localhost:5000/api/v1/place-order",{order:Cart},
      {headers}
    )
 alert(response.data.message)
      ...
      {Cart.length === 0 && (
        <div className='h-screen'>
          <div className='h-[100%] flex items-center justify-center flex-col'>
            ...
            <img src="https://cdn-icons-png.flaticon.com/512/2762/2762885.png" alt="empty cart" className='lg:h-[40vh]' />
          </div>
        </div>
      )}
          ...
          {Cart.map((items, i) => (
            <div className='w-full my-4 rounded flex flex-col md:flex-row p-4 bg-zinc-800 justify-between items-center' key={i}>
              ...
              <div className='w-full md:w-auto'>
                ...
                {items.desc && <p className='text-normal text-zinc-300 mt-2 hidden lg:block'>
